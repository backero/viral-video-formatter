...
export default function Page06_Hook({ data, onChange, errors }) {
  ...
  const toggleFactor = (val) => {
    ...
    onChange('physiologicalFactors', arr.includes(val) ? arr.filter((v) => v !== val) : [...arr, val])
  }
  ...
  return (
    <div className="pg">
      <div className="mb-7">
        <h2 className="text-[22px] font-bold text-body tracking-tight mb-1">Hook</h2>
        <p className="text-[14px] text-dim">Grab attention in the first 1–3 seconds</p>
      </div>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-col gap-2">
              ...
              return (
                <label key={h.value} className={`hook-card ${selected ? 'on' : ''}`}>
                  <input
                    ...
                    value={h.value}
                    checked={selected}
                    onChange={() => { onChange('viralHook', h.value); onChange('viralHookOther', '') }}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-[14px] text-body leading-snug">{h.label}</div>
            ...
            {/* Other */}
            <label className={`hook-card ${data.viralHook === 'other' ? 'on' : ''}`}>
              <input
                type="radio"
                name="viralHook"
                value="other"
              ...
              <div className="flex-1 min-w-0">
                ...
                {data.viralHook === 'other' && (
                  <input
                    ...
                    value={otherHook}
                    onChange={(e) => { setOtherHook(e.target.value); onChange('viralHookOther', e.target.value) }}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            rows={3}
          />
          <span className="fh">Must show contrast, motion, result, or disruption instantly</span>
          {errors?.visualHook && <span className="fe">{errors.visualHook}</span>}
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            ...
            <div className="text-[14px] text-body font-medium">Drag and drop files here or click to browse</div>
            <div className="text-[12px] text-dim mt-1">Max 10 MB · PDF, DOC, JPG, PNG, MP4, AVI and more</div>
          </div>
          {(data.visualHookFiles || []).length > 0 && (
            <div className="flex flex-col gap-1.5 mt-1">
              {data.visualHookFiles.map((f, i) => (
                <div key={i} className="flex items-center gap-3 px-3 py-2 bg-raised border border-wire rounded-lg">
                  <span className="flex-1 text-[13px] text-body truncate">{f.name}</span>
                  <span className="text-[12px] text-dim">{formatSize(f.size)}</span>
                  <button
                    type="button"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            rows={3}
          />
          <span className="fh">BIG, Bold — Can it be read while scrolling? Readable in &lt;1 second</span>
          {errors?.writtenHook && <span className="fe">{errors.writtenHook}</span>}
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            rows={3}
          />
          <span className="fh">Must reinforce outcome — does the spoken line add urgency, mistake, or curiosity?</span>
        ...
        <div className="flex flex-col gap-1.5">
          <label className="fl">
            What does the Hook Trigger / Physiological Factors <span className="text-bad">*</span>
          </label>
          ...
          {errors?.physiologicalFactors && <span className="fe">{errors.physiologicalFactors}</span>}
        </div>
      </div>
    </div>
  )
