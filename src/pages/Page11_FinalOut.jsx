import { FINAL_APPROVAL_MEMBERS, FOUNDER_APPROVAL_OPTIONS } from '../constants/formData'
...
export default function Page11_FinalOut({ data, onChange }) {
  ...
  const toggleMember = (val) => {
    ...
    onChange('finalApprovalMembers', arr.includes(val) ? arr.filter((v) => v !== val) : [...arr, val])
  }

  const needsCorrection = data.founderApproval && data.founderApproval !== 'APPROVED'

  return (
    <div className="pg">
      <div className="mb-7">
        <h2 className="text-[22px] font-bold text-body tracking-tight mb-1">Final Out</h2>
        <p className="text-[14px] text-dim">Include the final link of the video and get approval</p>
      </div>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <select className="fi-select" value={data.founderApproval || ''} onChange={set('founderApproval')}>
        ...
        {needsCorrection && (
          <div className="border-l-2 border-bad pl-4 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <label className={`check-card ${data.reCorrectionDone === 'YES' ? 'on' : ''}`}>
                <input
                  type="radio"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="fl">Approval after Re-Correction by Founder Only</label>
              <select className="fi-select" value={data.reApproval || ''} onChange={set('reApproval')}>
                <option value="">Please Select</option>
        ...
        <div className="info-box">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-3">
            {[
              ...
              ['Creator', data.creatorFirst ? `${data.creatorFirst} (${data.creatorDept || ''})` : '—'],
              ['Actor', data.actorFirst ? `${data.actorFirst} (${data.actorDept || ''})` : '—'],
              ...
              <div key={label}>
                <div className="text-[11px] text-dim uppercase tracking-wide mb-0.5">{label}</div>
