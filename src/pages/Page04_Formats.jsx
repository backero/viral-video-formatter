...
export default function Page04_Formats({ data, onChange, errors }) {
  ...
  const structures =
    tierKey && SCRIPT_STRUCTURES[tierKey] ? SCRIPT_STRUCTURES[tierKey] : [];
    ...
    <div className="pg">
      <div className="mb-7">
        ...
        <p className="text-[14px] text-dim">
          How the video is visually delivered on camera
        </p>
      </div>
      ...
      <div className="flex flex-col gap-5">
            ...
            {FORMAT_TIERS.map((tier) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <div className="flex-1 flex flex-col sm:flex-row items-start sm:items-center gap-1.5 sm:gap-2.5 min-w-0">
                    <span
                      className={`tier-badge whitespace-nowrap w-fit ${TIER_COLORS[key]}`}
                    >
                      {key}-TIER
                    </span>
                    <span className="font-semibold text-[13px] sm:text-[14px] text-body leading-tight sm:leading-normal">
        ...
        {structures.length > 0 && (
          <div className="flex flex-col gap-1.5">
            ...
            <div
              className={`grid gap-2 ${structures.length > 3 ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1"}`}
            >
              {structures.map((s) => {
        ...
        {tierKey === "F" && (
          <div className="bg-bad/6 border border-bad/30 rounded-xl p-5">
            <p className="text-bad font-semibold mb-1.5">
              F-TIER structures don't hold retention consistently.
            </p>
            <p className="text-dim text-[14px] leading-relaxed">
              Avoid: vague motivational talk · generic advice with no mechanism
